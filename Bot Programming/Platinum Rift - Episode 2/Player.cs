...
class Player
{
	static void Main(string[] args)
	{
		...
		while (true)
		{
			...
			Console.WriteLine(string.Join(" ", commands.ToArray())); //prima riga per i comandi di movimento, seconda riga non più utilizzata (per i dettagli vedere dichiarazione dello statemant)
...
#warning No support for multiple squads at same location at this time
	...
	private static GameState loadInitialStateFromConsole()
	{
		...
		gameState.MyId = int.Parse(inputs[1]); //player ID (0 or 1)
		//Log("MyId: {0}, Platinum: {1}", gameState.MyId, gameState.MyPlatinum);

		int zoneCount = int.Parse(inputs[2]); // numero di zone nella mappa
		int linkCount = int.Parse(inputs[3]); // quantità di collegamenti nelle zone

		gameState.Zones = new Zone[zoneCount];
		for (int i = 0; i < zoneCount; i++)
		{
			inputs = Console.ReadLine().Split(' ');
			int zoneId = int.Parse(inputs[0]); //  ID di questa zona (tra 0 e zoneCount-1)
			int platinumSource = int.Parse(inputs[1]); // a causa della nebbia => sempre 0
	...
	private static void updateGameStateFromConsole(GameState gameState)
	{
		...
		for (int i = 0; i < gameState.Zones.Length; i++)
		{
			...
			int zId = int.Parse(inputs[0]); // l'ID di questa zona
			gameState.Zones[zId].OwnerId = int.Parse(inputs[1]); // player che possiede la zona (-1 otherwise)

			var podsp0 = int.Parse(inputs[2]); // player 0's PODs in questa zona
			var podsp1 = int.Parse(inputs[3]); // player 1's PODs in questa zona
			gameState.Zones[zId].MyPods = gameState.MyId == 0 ? podsp0 : podsp1;
			gameState.Zones[zId].TheirPods = gameState.MyId != 0 ? podsp0 : podsp1;

			gameState.Zones[zId].Visible = int.Parse(inputs[4]) == 1; // 1 se una delle tue unità può vedere questa tessera, altrimenti 0
			gameState.Zones[zId].PlatinumSource = int.Parse(inputs[5]); // la quantità di platino che questa zona può fornire (0 se nascosta dalla nebbia)
...
public class Dijkstra
{
	...
	{
		if (to >= _nodes.Length)
			return null;//Nessun percorso per la destinazione
